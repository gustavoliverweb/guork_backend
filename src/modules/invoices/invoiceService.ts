...
export class InvoicesService {
  ...
  async createInvoices(data: InvoiceInput): Promise<InvoiceModel> {
  ...
  async getLastInvoice(): Promise<InvoiceModel> {
    const record = await this.invoicesRepository.findLastInvoice();
    if (!record) throw new Error("Invoices not found");
    return record;
  }

  async getInvoicesByRequesterId(
    id: string
  ): Promise<{ rows: InvoiceModel[] }> {
  ...
  async updateInvoices(id: string, data: InvoiceInput): Promise<InvoiceModel> {
